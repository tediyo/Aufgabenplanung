...
const GoogleLoginButtonFallback = ({ onSuccess, onError, text = "Continue with Google", className = "", disabled = false }) => {
  const handleGoogleLogin = () => {
    if (disabled) return;
    
    // Show a prompt for testing
    const email = prompt('Enter your email for testing (or press Cancel to skip):');
    if (email) {
      const mockUser = {
        id: 'test-' + Date.now(),
        email: email,
